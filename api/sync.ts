  ...
  try {
    ...
    const ensureSchemaForGet = async () => {
      ...
      try {
        ...
        await sql`ALTER TABLE justificativas ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP`;
      } catch (alterErr) {
      ...
      try {
        ...
        await sql`ALTER TABLE pontos ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP`;
      } catch (alterErr) {
    ...
    const ensurePontosSchema = async () => {
      await sql`
        CREATE TABLE IF NOT EXISTS pontos (
          id TEXT PRIMARY KEY,
          cooperado_id TEXT NOT NULL,
      ...
      try {
        await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_pontos_id ON pontos(id)`;
      } catch (indexErr) {
        console.log('[sync] Aviso ao garantir indice de pontos.id:', indexErr);
      }
      ...
      try {
        ...
        console.log('[sync] Aviso ao ajustar schema de pontos:', alterErr);
      }
    };

    if (action === 'sync_ponto' || action === 'delete_ponto') {
      await ensurePontosSchema();
    }
    ...
    try {
      await sql`ALTER TABLE managers ADD COLUMN IF NOT EXISTS preferences TEXT`;
    } catch (alterErr) {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS cooperados (
        ...
        producao_por_cpf TEXT DEFAULT 'Não',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      );
    `;
    ...
    try {
      await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_cooperados_id ON cooperados(id)`;
    } catch (alterErr) {
      console.log('[sync] Aviso ao garantir indice de cooperados.id:', alterErr);
    }
    ...
    try {
      ...
      await sql`ALTER TABLE cooperados ADD COLUMN IF NOT EXISTS producao_por_cpf TEXT DEFAULT 'Não'`;
      await sql`ALTER TABLE cooperados ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP`;
    } catch (alterErr) {
    ...
    try {
      ...
      await sql`ALTER TABLE hospitals ADD COLUMN IF NOT EXISTS endereco TEXT`;
      await sql`ALTER TABLE hospitals ADD COLUMN IF NOT EXISTS permissoes TEXT`;
      await sql`ALTER TABLE hospitals ADD COLUMN IF NOT EXISTS setores TEXT`;
      await sql`ALTER TABLE hospitals ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP`;
    } catch (alterErr) {
    ...
    if (action === 'sync_cooperado') {
      ...
      const producaoPorCpf = c.producaoPorCpf === 'Sim' ? 'Sim' : 'Não';
      ...
      const result = await sql`
        ...
        VALUES (
          ...
          ${c.categoriaProfissional || null}, ${c.matricula || null}, ${c.status || 'ATIVO'}, ${producaoPorCpf}, CURRENT_TIMESTAMP
        )
        ON CONFLICT (id) DO UPDATE SET
          name = ${c.nome},
          cpf = ${c.cpf || null},
    ...
    if (action === 'sync_justificativa') {
      ...
      const result = await sql`
        ...
        VALUES (
          ...
          ${j.dataPlantao || null}, ${j.entradaPlantao || null}, ${j.saidaPlantao || null}, ${j.dataAprovacao || null}, CURRENT_TIMESTAMP
        )
        ON CONFLICT (id) DO UPDATE SET
    ...
    if (action === 'sync_ponto') {
      ...
      await sql`
        INSERT OR IGNORE INTO pontos (id, cooperado_id, created_at)
        VALUES (${p.id}, ${p.cooperadoId}, CURRENT_TIMESTAMP)
      `;

      const result = await sql`
        UPDATE pontos SET
          codigo = ${p.codigo || null},
          cooperado_id = ${p.cooperadoId},
          cooperado_nome = ${p.cooperadoNome || null},
          "date" = ${p.data || p.date || null},
          tipo = ${p.tipo || null},
